...
from preprocessing import load_adl
...
def llm_for_refactoring(adl, new_requirement):
    ...
    sys_instruct="""You are an expert for refactoring the architecture design model in Wright#. Your task is to modify the design model based on the new requirement. The Output only contains the script of refactored architecture design model."""
...
def append_to_excel(adl_file_name, new_requirement, result, run_id, file_path):
    ...
    print(f"Execution {run_id} logged in Excel for {adl_file_name}")

# Load Excel data
input_file = "../Refactoring_data.xlsx"  # Change this to your actual Excel file name
df = pd.read_excel(input_file, engine="openpyxl")
df = df.iloc[5:61]  # Select rows from index 0 to 61 (inclusive)
...
for index, row in df.iterrows():
    ...
    for run_id in range(1, 9):  # Runs each instance 8 times
        try:
            response = llm_for_refactoring(adl, new_requirement)
            refactored_adl = get_adl(response.text)
            final_result = validation_verification(refactored_adl, new_requirement)
            output_file = "baseline_gemini2.0flash_results.xlsx"
            ...
            print(f"Error in execution {run_id} for {adl_file_name}: {str(e)}")
            continue
